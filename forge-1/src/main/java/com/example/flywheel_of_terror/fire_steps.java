...
public class fire_steps {
   // Per-player fire-trail timer (NBT "fire_steps_tics"), now decremented server-authoritatively.
   public static void set_tics(Player player, int value) {
      state.putInt(player, "fire_steps_tics", value);
   }
   ...
   public static void spawn_fire(PlayerTickEvent event) {
      ...
      if (!player.level().isClientSide()) {
         int tics_of_event = state.getInt(player, "fire_steps_tics");
         if (tics_of_event > 0) {
            tics_of_event--;
            if (player.onGround()) {
            ...
            state.putInt(player, "fire_steps_tics", tics_of_event);
         }
      }
   }
}
