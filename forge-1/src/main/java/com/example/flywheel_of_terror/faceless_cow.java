...
public class faceless_cow extends Cow {
   public int tics_to_delete = this.random.nextInt(200, 300);

   public faceless_cow(EntityType<? extends Cow> type, Level level) {
      super(type, level);
      this.tics_to_delete = this.random.nextInt(200, 300);
   }

   public static Builder createAttributes() {
      return Cow.createAttributes().add(Attributes.MAX_HEALTH, 100.0).add(Attributes.FOLLOW_RANGE, 35.0).add(Attributes.MOVEMENT_SPEED, 0.01);
   }
